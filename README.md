...
- A **Pause Duration** select (`15 minutes` / `30 minutes` / `1 hour` /
  `2 hours` / `4 hours` / `8 hours` / `24 hours`) — buttons can't prompt for
  input when pressed, so this is what pressing **Pause** actually uses: pick a
  preset once, or change it right before pressing Pause. Persists across
  restarts.

Pressing **Pause** uses that entity's currently-selected preset; the `pause`
service lets you override it with a one-off custom duration per call instead
(e.g. from an automation), without changing the select's saved value.
...
- Pick a preset on `select.alex_pause_duration` (or
  `select.all_kids_pause_duration`) to set how long the next press pauses
  for, then press **Pause**. Both live under the same device, so they show up
  together in the UI.
- Press any child's **Pause**/**Resume** button, or the **All Kids** versions,
  from the UI or automations.
- One-off custom duration that doesn't match a preset, without touching the
  select's saved value — target the Pause button entity directly:
